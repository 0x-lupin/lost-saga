...
export class Player {
    ...
    update(delta, moveInput, platforms, arenaBounds, obstacles = [], enemies = []) {
        ...
        if (this.isDashing) {
            ...
            if (this.dashTimeLeft <= 0) {
                ...
                this.velocity.y = 0; // Stop any vertical momentum from dash entry if we add it later
            } else {
                // Override movement input during dash
                moveInput = { x: this.dashDirection.x, z: this.dashDirection.z };
                // Use dash speed
                var currentSpeed = this.dashSpeed;

                // Trail effect logic could go here
            }
        }

        // Determine effective speed (Dash or normal Move)
        const speed = this.isDashing ? this.dashSpeed : this.moveSpeed;

        this.isMoving = moveInput.x !== 0 || moveInput.z !== 0;

        if (!this.isDashing && this.isAttacking && this.isGrounded) {
            // Slower movement while attacking
            // (Optional, keeps alignment with typical mechanics, but sticking to valid update for now)
        }

        // Apply movement
        this.mesh.position.x += moveInput.x * speed * delta;
        this.mesh.position.z += moveInput.z * speed * delta;
    ...
    dash(inputDir) {
        ...
        this.isDashing = true;
        this.isInvulnerable = true; // Enable I-frames
